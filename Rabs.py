...
def add_camera(data: CameraInput,  current_user: User = Depends(get_current_active_user)):
    try:
        ...
        allow_polygonal_points = mongo_handler.has_polygonal_points(email=data.email)
        ...
        update_status = mongo_handler.save_user_to_mongodb_truck({"email": data.email, "cameras": existing_cameras}) \
            if allow_polygonal_points else \
            mongo_handler.save_user_to_mongodb({"email": data.email, "cameras": existing_cameras})
        ...
        if update_status:
            ...
            return {
                ...
                "camera_id": data.camera_id,
                "rtsp_link": data.rtsp_link,
                "polygonal_points": new_camera.get("polygonal_points", "Not Allowed")  # Show status
            }
        else:
...
def remove_camera(email: str, camera_id: str, current_user: User = Depends(get_current_active_user)):
    try:
        ...
        existing_cameras = mongo_handler.fetch_camera_rtsp_by_email(email) or []
        ...
        if result.modified_count > 0:
            logger.info(f"Camera {camera_id} removed successfully for {email}")
            return {"message": "Camera removed successfully", "email": email, "camera_id": camera_id}
        else:
...
async def start_streaming(model_input: ModelInput,background_tasks: BackgroundTasks,current_user: User = Depends(get_current_active_user)):
    try:
        ...
        server_address = os.getenv("SERVER_ADDRESS")  #"http://0.0.0.0:8000"  # Update with your actual server address
...
async def start_single_camera_streaming(camera_id: str,rtsp_link: str,model_input: ModelInput,background_tasks: BackgroundTasks, current_user: User = Depends(get_current_active_user)):
    try:
        ...
        if  mongo_handler.has_polygonal_points(email = current_user.email) == True:
            camera_system = SingleCameraSystemTruck(camera_id=camera_id, rtsp_link=rtsp_link,
                email=current_user.email, model_path= model_input.model )

        else:
            camera_system = SingleCameraSystem(camera_id=camera_id, rtsp_link=rtsp_link,
                email=current_user.email, model_path= model_input.model )
...
async def stop_single_camera_streaming(current_user: User = Depends(get_current_active_user)):
    try:
