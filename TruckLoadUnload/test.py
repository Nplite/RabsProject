...
from RabsProject.camera_system import MultiCameraSystemTruck, SingleCameraSystemTruck
# from A import MultiCameraSystemTruck
...
def add_user(user: UserInput):
    ...
    success = mongo_handler.save_user_to_mongodb_truck(user_data)
...
async def start_streaming(current_user: User = Depends(get_current_active_user)):
    ...
    camera_system = MultiCameraSystemTruck(email=current_user.email, model_path='yolov8s.pt')
...
@app.get("/tracking_statistics")
def get_tracking_statistics(email: str = Query(..., description="User's email to fetch tracking statistics")):
    """Retrieve tracking data statistics for a given email."""
    multi_camera_systems = {}
    try:
        if email not in multi_camera_systems:
            multi_camera_systems[email] = MultiCameraSystemTruck(email)

        multi_camera_system = multi_camera_systems[email]
        statistics = multi_camera_system.get_all_statistics()

        if not statistics:
            raise HTTPException(status_code=404, detail="No tracking statistics found")

        return {
            "email": email,
            "tracking_statistics": statistics
        }

    except Exception as e:
        logging.error(f"Error fetching tracking statistics: {str(e)}")
